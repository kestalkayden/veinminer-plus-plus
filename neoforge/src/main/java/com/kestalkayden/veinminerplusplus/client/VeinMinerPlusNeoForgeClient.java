...
public final class VeinMinerPlusNeoForgeClient {
    ...
    private static void onClientTick(ClientTickEvent.Post event) {
        ...
        while (KEY_TOGGLE.consumeClick()) {
            ...
            // Client-only chat line (displayClientMessage(component, false) routes to the chat HUD,
            // never the server) so only this player sees the on/off feedback.
            client.player.displayClientMessage(
    ...
    /**
     * Send a C2S payload over the active play connection: wrap it in a
     * {@link ServerboundCustomPayloadPacket} and send it directly via the connection.
     */
