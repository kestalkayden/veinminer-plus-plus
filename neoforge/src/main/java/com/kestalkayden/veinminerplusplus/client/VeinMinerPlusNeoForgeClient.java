...
public final class VeinMinerPlusNeoForgeClient {
    ...
    private static void onClientTick(ClientTickEvent.Post event) {
        ...
        while (KEY_TOGGLE.consumeClick()) {
            ...
            // Client-only chat line (Player#displayClientMessage with isActionBar=false routes to
            // the chat HUD, never the server) so only this player sees the on/off feedback.
            client.player.displayClientMessage(
     ...
     * not used here — the payload is wrapped in a {@link ServerboundCustomPayloadPacket} and sent
     * directly via the connection.
     */
